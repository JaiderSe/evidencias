...
SELECT COUNT(DISTINCT Code) as paises_Registrados
FROM country;
select count(*) from countrylanguage where((Language)='French');
select name from country where((Continent)='South America') order by IndepYear asc;
SELECT Language FROM countrylanguage WHERE CountryCode = (SELECT Code FROM country WHERE Name = 'Andorra');
